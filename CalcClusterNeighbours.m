function [all, in, out] = CalcClusterNeighbours( Nxc, Nyc, Nx, Ny )
%-------------------------------------------------------------------------%
...
%-------------------------------------------------------------------------%
    ...
    for j=1:Nx*Ny
        ...
        t(j,3)=Ny*(Mod(ceil(j/Ny)-1,Nx,1)-1)+Mod(j,Ny,1);   %Up neighbour
        t(j,4)=Ny*(Mod(ceil(j/Ny)+1,Nx,1)-1)+Mod(j,Ny,1);   %Down neighbour
    end
    all = t;
